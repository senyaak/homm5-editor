...
export function pandoraBehaviourLua(): string {
  return [
    ...
    '\t-- WHAT THE GAME ITSELF DOES WITH A SPELL, in three cases.',
    '\t--',
    '\t-- H5ECanLearnSpell is the engine\'s whole question, the one the spell shop',
    '\t-- and the shrine go through: the school against the hero\'s kind, the SKILL',
    '\t-- of that school, and the level the record asks for. A wizard with no Dark',
    '\t-- Magic is refused Curse, a knight is refused war cries, a barbarian is',
    '\t-- refused everything but them - and a war cry itself wants hero level 2, 6',
    '\t-- or 11 by its own level.',
    '\t--',
    '\t-- REFUSED FOR WHAT HE IS, he is PAID: 1000 experience per level of the',
    '\t-- spell, the rate the game\'s own shrines pay a barbarian handed magic.',
    '\t-- Refused for what he has not got YET - a skill, a level - the spell is',
    '\t-- LOST, exactly as it is at a shrine. H5ECanHoldSpell is what tells the',
    '\t-- two apart: it asks the school gate alone, which is the question "is',
    '\t-- this even his kind of magic".',
    '\t-- SO IT IS THE SAME RULE FOR EVERY KIND, and adventure magic has no path',
    '\t-- of its own any more. A barbarian is paid for a fireball and for Town',
    '\t-- Portal alike, a knight for a war cry, anybody but a dwarf for a RUNE -',
    '\t-- and a dwarf learns the rune if his Runelore is far enough along, or',
    '\t-- loses it if it is not.',
    ...
    '\t\t\telseif H5ECanHoldSpell(hero, s[1]) == nil then',
    '\t\t\t\t-- PAID FOR WHAT HE IS, not for how young he is. A hero handed magic',
    '\t\t\t\t-- of a kind that is not his is paid the way the shrines pay a',
    '\t\t\t\t-- barbarian; a hero who has not grown into the spell yet loses it.',
