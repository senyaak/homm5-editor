...
export function pandoraBehaviourLua(): string {
  return [
    ...
    '\t-- announced on the other side of the map, to somebody who did not touch',
    '\t-- it. So the box only speaks when the hero who touched it belongs to the',
    '\t-- player whose turn it is, which is what the shipped scripts use',
    '\t-- GetCurrentPlayer for. Anyone else - an AI, or the other side in a',
    '\t-- hotseat - opens it in silence and gets everything just the same.',
    '\tlocal player = GetObjectOwner(heroName);',
    '\tif IsAIPlayer(player) == true or player ~= GetCurrentPlayer() then',
    ...
    '\t-- the same reason the question does. GetObjectOwner is asked again here',
    '\t-- rather than carried: a fight sits between the two, and the hero can',
    '\t-- have changed hands or died in it.',
    '\tif box.given ~= nil and GetObjectOwner(hero) == GetCurrentPlayer() then',
