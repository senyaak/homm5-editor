...
//
// WHY IT IS IN HERE. We are porting the generator to TypeScript, and a port can
...
/** `mov eax,[draw counter]; ret` — five bytes, a whole instruction. */
...
/** Append to the oracle's own file — machine-readable, and never the game's. */
...
/** Append one line to a file beside us. Silent if it cannot. */
static void log_to(const WCHAR *file, const char *text) {
  WCHAR path[MAX_PATH];
  beside_us(file, path);
...
static void log_line(const char *text) { log_to(L"homm5-editor.log", text); }

/**
 * The oracle's file, kept apart from the extension's diary on purpose: one is
 * read by a person to see whether the mod loaded, the other is read by a tool
 * that compares it with a port. Mixing them would make the second one a parsing
 * exercise for no reason.
 */
static void rmg_log(const char *text) { log_to(L"homm5-editor-rmg.log", text); }
 ...
 *                       one the game picked
 *
 * Deliberately a second file rather than a section of the first: the effects
 * file is written by the editor whenever a mod changes, and an instrument that
 * a person turns on for an afternoon has no business being rewritten by that.
 */
static void load_rmg_config(void) {
  WCHAR path[MAX_PATH];
  beside_us(L"homm5-editor-rmg.txt", path);
  HANDLE h = CreateFileW(path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
                         FILE_ATTRIBUTE_NORMAL, NULL);
  if (h == INVALID_HANDLE_VALUE) return;
 ...
 *
 * Not a detour: the five bytes of a `call rel32` are exactly one instruction, so
 * this replaces the destination and touches nothing around it. It is checked by
 * where the call currently GOES rather than by the bytes it is made of — the
 * displacement is relative and the target is an address, so "this call reaches
 * the seed setter" is a claim that survives a different load base, and
 * comparing raw bytes would not.
 */
static int patch_call(DWORD siteRva, DWORD targetRva, void *to, const char *what) {
  ...
  if (site[0] != 0xE8 || site + 5 + *(int *)(site + 1) != base + targetRva) {
    log_line(what);
    log_line("  that call does not go where we think - not patching");
    return 0;
  }
  DWORD old = 0;
...
static void *patch_indirect_call(DWORD siteRva, void *hook, const char *what) {
  ...
  if (site[0] != 0xFF || site[1] != 0x15) {
    log_line(what);
    log_line("  that is not an indirect call - not patching");
    return NULL;
  }
 ...
 * any other reason still stops us.
 */
static int install_rmg_oracle(void) {
  ...
  *(DWORD *)(head + 1) = (DWORD)(base + RMG_COUNTER_FIELD_RVA);
  ...
  g_rmgCounter = (CounterFn)detour(RMG_COUNTER_RVA, head, &rmg_counter_hook, "rmg draw counter");
  ...
  if (g_rmgForceSeed && !g_time) log_line("  the seed cannot be forced - it will be the clock's");
  return 1;
}
...
BOOL WINAPI DllMain(HINSTANCE self, DWORD reason, LPVOID reserved) {
  ...
  // The generator's oracle, only when its config is there. It hooks nothing the
  // artifact work hooks, so it stands or falls on its own.
  load_rmg_config();
  if (g_rmgWanted) {
    log_line(install_rmg_oracle() ? "rmg oracle installed" : "rmg oracle NOT installed");
  }
