...
BOOL WINAPI DllMain(HINSTANCE self, DWORD reason, LPVOID reserved) {
  ...
  if (rmg_host_is_editor()) {
    ...
    // The mask's other half, under its own word: registration runs while the
    // objects are being placed, so it cannot ride the minimap probe's window.
    if (g_rmgMask)
      log_line(install_mask_probe() ? "mask probe installed" : "mask probe NOT installed");
    if (g_rmgCliWanted)
  ...
  // And the mask's other half, for the same reason: the editor's copies of
  // these three functions never ran through a whole ordered run, and both maps
  // the question is about were generated by the game.
  if (g_rmgMask)
    log_line(install_mask_probe_game() ? "mask probe installed" : "mask probe NOT installed");
  return TRUE;
}
