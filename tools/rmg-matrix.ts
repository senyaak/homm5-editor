...
function tables(): void {
  ...
  const p = [
    ...
    ['size', '-size', `0..6 = ${MAP_SIZES.join(', ')} (${sizeNames(gameInstall()).map((s) => s.replace('MAP_SIZE_', '')).join('/')}), lifted by the fit`],
    ...
    ['resource', '-resource', `0..4 = ${multiplierNames(gameInstall()).join(', ')}`],
    ['exp', '-exp', `0..4 = ${multiplierNames(gameInstall()).join(', ')}`],
