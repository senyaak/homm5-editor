...
// Said, never guessed from the checkout's position (tools/game-dir.ts): this
// runs inside `npm test`, so a machine that has not said where the game is
// skips this half in so many words rather than failing.
const game = gameDirIfAny();
const exePath = game ? join(game, 'bin', 'H5_Game_H5E.exe') : null;
let pe;
if (!exePath) {
  console.log('  skip  the constants — no game said (pass --game <dir> or set HOMM5_GAME)');
} else if (!existsSync(exePath)) {
  console.log(`  skip  the constants — ${exePath} is not there; \`npm run unwrap-exe\` makes it`);
} else {
  pe = PEFile.read(exePath);
}
