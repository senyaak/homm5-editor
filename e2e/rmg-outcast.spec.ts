...
async function addObject(page: Page, search: string, name: RegExp, min: number, max: number | null, guard: number): Promise<void> {
  ...
  await page.locator('#op-list .op-opt', { hasText: name }).click();
...
test('Jebus Outcast, drawn click by click, saved, and generated from', async () => {
  ...
  expect(saved.zones[0]!.objects).toEqual([{ href: '/MapObjects/Dragon_Utopia.(AdvMapBuildingShared).xdb#xpointer(/AdvMapBuildingShared)', min: 1, max: 1, guardStrenght: 30 }]);
  ...
  expect(xdb).toContain('Dragon_Utopia');
});
