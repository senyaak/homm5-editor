...
- [engineInternals/FIRST_AID_TENT.md](engineInternals/FIRST_AID_TENT.md) — the first aid tent as a mechanism: what it heals, how many uses it has, where a term of ours joins each, and how the branch's scripted perk was written as Lua on both sides of a battle.
- [ARTIFACT_EFFECTS.md](ARTIFACT_EFFECTS.md) — what data, script and the exe each control.
- [engineInternals/SPELLS.md](engineInternals/SPELLS.md) — a spell of our own: the five places its table's size is declared, the twenty-two fields a spell document has (and why none of them says what it DOES), how far a new one gets by itself, the gate that refuses it, the three damage shapes and the pair of flags that chooses between them, the tiles an area covers, and the **Spells…** window a person makes one in.
...
- [PANDORA_BOX.md](PANDORA_BOX.md) — the box an author fills in: what it can hold, what that is worth (and which of those prices are the game's and which are ours), where the contents live, and what actually ships inside the map.
...
- [PERFORMANCE.md](PERFORMANCE.md) — the performance log: what got faster, when, by how much — kept out of the changelog.
