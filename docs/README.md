...
- [engineInternals/SPELLS.md](engineInternals/SPELLS.md) — a spell of our own: the five places its table's size is declared, the twenty-two fields a spell document has (and why none of them says what it DOES), how far a new one gets by itself, the gate that refuses it, and the bridge that hands its cast to Lua.
