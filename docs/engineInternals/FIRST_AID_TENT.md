...
**The third is adventure-map shaped**, and Lua can have it: a tent destroyed in a
battle is rebuilt afterwards. `COMBAT_RESULTS_TRIGGER` fires after a combat with
the combat index; `GetSavedCombatArmyHero` names the heroes, `HasHeroSkill` asks
whether one holds the perk, and `HasHeroWarMachine` / `GiveHeroWarMachine` do the
rest. It belongs in the mod's own `scripts/advmap-common.lua`, which runs on
**every** adventure map, the game's own included, and where triggers stack rather
than replace — both measured, in
