...
### The creature table ends where the executable says

The guard setter draws from the creature table as the executable holds it:
its first N rows, N being the ceiling compiled in (180 shipped, `mov eax,
180` in an accessor — `creature-limit.ts` knows the sites, and a units mod
patches them to its own count). The port read the whole file. On an install
with a mod whose table runs past the ceiling that is a different pool: on
16.09 a Small S1P2Z3K5.1 (seed 610809267) generated by the editor of the
Steam install — 11 archives mounted, a 184-row table, the game's executable
patched to 181 and the map editor's at 180 — came back with three guards
the port had drawn differently (Druid Elder, Lich, Water Elemental against
Rune Mage, Unicorn, Rune Mage), everything else byte for byte. Cut to the
editor's 180 the port reproduces the editor's map; cut to the game's 181
the H3 sharpshooter (row 180) joins the pool and the three guards move —
which is what the game's own dialog would draw on that install, if its
generator is the editor's with the ceiling raised (not batchable, so not
measured). `RmgExeTables.creatureCount` now carries the ceiling read out of
the executable the install names, and `chain.ts` slices the table to it.
The two executables of one install can disagree — the units mod patches
the game's, the map editor's copy is made with the import alone — so a
modded install's batch oracle is one row short of its game.
