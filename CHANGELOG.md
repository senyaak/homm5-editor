...
modelled. Beside the designed matrix there is now a random suite
(`npm run rmg-random-suite`): a few orders a size with every input drawn,
generated by the engine and replayed by the port, new orders every run and
any run repeatable by its seed. The generator is a module with one door
(`src/rmg/index.ts`: the dialog's choices, the templates it would offer, an
order in its units, a `.h5m` out) that the application imports: **Random
...
every hero of the races the players come out as. And a **Mods** box: on,
the generator reads the install with its archives mounted, as the game does
— a mod's creatures, templates and heroes are in; off, the game's data
alone, a vanilla map from a modded install. A map opened from a `.h5m` of ours is now titled after the
